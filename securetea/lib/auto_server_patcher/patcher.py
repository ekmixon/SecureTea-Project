...
class ConfigPatcher(object):
    ...
    def __init__(self, debug=False, to_patch=None):
        ...
        if os_name := utils.categorize_os():
            try:
    ...
    def patch(self):
        ...
        for path in self.os_config_data:  # iterate over the configuration
            patch_this = any(req_patch in path for req_patch in self.to_patch)
            if patch_this:
                self.logger.log(f"Patching: {str(path)}", logtype="info")
                    ...
                    for rep_text in self.os_config_data[path]["config"].keys():
                        ...
                        if rep_text in line:
                            ...
                            if not in_front:  # not in forward refrencing
                                ...
                                self.logger.log(f"New config line: {new_config_line}", logtype="info")
                ...
                self.logger.log(f"Patched: {str(path)}", logtype="info")
